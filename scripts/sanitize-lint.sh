...
EXCLUDE_GLOBS=(
  ...
  ":(exclude)scripts/pios-auto-commit.sh"  # comments reference patterns it resets
)
...
_scan_staged() {
  ...
  # Exclude self-referential files — these legitimately contain the patterns
  # as source-of-truth definitions or as documentation examples.
  # Keep this list in sync with EXCLUDE_GLOBS above.
  local label pattern total_hits=0
  local staged_files
  staged_files=$(git diff --cached --name-only --diff-filter=ACM 2>/dev/null \
                 | grep -vE '^(scripts/sanitize-lint\.sh|scripts/pios-auto-commit\.sh|\.githooks/(pre-commit|pre-push|commit-msg)|docs/oss-safe-coding\.md)$')
...
_scan_history() {
  ...
  local exclude_paths=(
    ...
    ":(exclude)scripts/pios-auto-commit.sh"
  )
  for entry in "${PATTERNS[@]}"; do
    label="${entry%%:*}"
    pattern="${entry#*:}"
