...
# ⚠️ 2026-04-29 plan c：删除 bundle → Vault 反向同步段，root cause 在这里。
# 历史：2026-04-26~28 间这段 cp "$ROOT/backend/tools/"*.sh "$_VAULT_TOOLS/" 把
# 打包冻结的旧 backend/tools 强制覆盖 Vault Pi/Tools/，结果 21 个 commit 静默回退
# （notify-wechat.sh 加 --media + mirror 后下次 install 还原成无 mirror 70 行版；
# 4/28 早 daily-diary-image 出图链路因此哑），见 verify-pios-tools-regression-fix-2026-04-29。
#
# 新模型：Vault 是 SSoT；首装 bootstrap 由 pios-installer.js 13b 一次性 cp
# bundle/backend/tools → Vault/Pi/Tools 完成；持续更新走 git commit + Syncthing。
# install-app.sh 只更新 .app payload，不再 touch Vault。
echo "[install-app] vault sync skipped (Vault is SSoT since plan c, 2026-04-29)"
