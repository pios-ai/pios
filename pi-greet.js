...
const VAULT_ROOT  = process.env.PIOS_VAULT || path.join(require('os').homedir(), 'PiOS');
...
const GREET_TIMEOUT_MS = 5000; // GPT 硬超时；超了 fallback
...
function _buildGreetPrompt({ deltaMs, piStateNow, recentBeats, events, ownerName }) {
  ...
  return `你是 Pi。不是助手角色，是 ${ownerName} 的常驻伙伴。现在 ${ownerName} 离开 ${deltaStr}后回来了（${tod}）。
...
async function _generateAndSpeakGreeting({ deltaMs, bands, mainWindow, tag }) {
  ...
  try {
    ...
    const gptPromise = client.chat(`（${ownerName} 刚回来）`, { systemPrompt, timeout: GREET_TIMEOUT_MS, temperature: 0.9 });
    ...
    if (greeting) usedGpt = true;
  } catch (e) {
    console.warn(`${tag} GPT fail: ${e.message} → fallback 模板`);
  }
