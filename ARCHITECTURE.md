...
Tracked gaps for v0.8.1. Resolved items are listed below the table.
...
- ✅ **TTS streaming for long replies (v0.8.1)**: `backend/qwen-tts.js`'s `speak()` now accepts an `onChunk` callback. When provided, each chunk's WAV is emitted in-order as soon as synthesis completes (synthesis itself stays parallel via `Promise.all`-like fan-out, so total wall-time is unchanged). `main/ipc-handlers/voice.js` wires the callback to `voice:tts:chunk` IPC events; renderer's `AudioQueue` enqueues each chunk as it arrives. First-byte latency for a multi-chunk reply drops from `N × per-chunk-time` to `1 × per-chunk-time`. Trade-off: voice-filter is now applied per-chunk in streaming mode (echo continuity across chunk boundaries is slightly broken), accepted as a worthwhile UX trade. Buffer-mode (when `onChunk` is omitted) is unchanged. Resolves issue #8.
