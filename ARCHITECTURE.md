...
Tracked gaps for v0.8.2. Resolved items are listed below the table.

| # | Gap | Impact | Priority |
|---|-----|--------|----------|
| 1 | Naming inconsistency: pios / pi-browser / PiOS / PiBrowser | Four names for overlapping things across UI / docs / packages | Low |
| 2 | `renderer/app.js` (7500-line legacy monolith) has no automated UI test | UI regressions in this layer are caught by owner dogfood, not CI. New renderer code goes in `renderer/lib/` (UMD + sibling test); the legacy file stays as-is — refactoring it risks the daily-driver UI. | Medium |
| 3 | DMG is not code-signed / notarized | macOS Gatekeeper blocks first launch; user has to manually approve | High (for distribution) |
| 4 | Setup wizard requires Claude Code CLI to be pre-installed | Not truly one-click; user must run `npm i -g @anthropic-ai/claude-code && claude auth login` first | Medium |

### Resolved (v0.7.0 → v0.8.2)
...
- ✅ **`main.js` 7469 → 497 (-93.3%) + 26 modules ≤800 (2026-04-29)**: 单 entry 退化为 thin shell（`app.whenReady` + `_browserCtrlState` 拼装）；逻辑全部下放到 `main/` 26 个职责单一模块。HTTP API（原 2871 行）拆成 shell + 5 个 sub-handler（GET×2 / POST×2 / browser-cmds），通过 ctx 包传 state + `res.writableEnded` 短路链式分派。引擎层 / Tab/Window / 通知 / 后台子系统各自独立，新加 IPC handler 5 步流程见 `main/README.md`。途中修了 2 个 latent bug（browser-control-api destructure 漏 handlePiEvent + bca-get-1 漏 require ./browser-control-api-auth）。验收 6/6 全过：build:dir + install-app.sh + 6 个 endpoint 实测。详见 `Cards/active/followup-pios-mainjs-split.md` tick 1-8。
- ✅ **Test infrastructure overhaul (v0.8.0)**: migrated from a single 13-case custom-harness smoke (`test/p6-smoke-test.js`) to the `node:test` runner with per-module file layout — 108 tests across 22 files. Coverage spans pure backend modules (pi-greet / pi-route / pi-chitchat / pi-speak / pi-persona / atomic-write / voice-filter / host-resolve / vault-context / pios-installer / presence), `main/*` via an electron-mock layer (`session-manager`, `installer-bridge`, `ipc-handlers/bookmarks`), and `renderer/lib/*` UMD modules via jsdom for DOM-using helpers. GitHub Actions CI (`.github/workflows/ci.yml`) runs sanitize-lint + unit + integration on a node 20 + 22 matrix plus a separate coverage job (`--experimental-test-coverage`); the pre-commit hook runs the unit subset (~3s) when staged changes touch `backend/`, `main/`, or `test/unit/`. Conventions documented in `test/CONVENTIONS.md`: every new `backend/<x>.js` or `main/<x>.js` ships with a sibling `test/unit/<x>.test.js`; new renderer code goes in `renderer/lib/<x>.js` (UMD wrapper) with a sibling test. Decision deliberately deferred: no Playwright e2e — `renderer/app.js` legacy monolith stays dogfood-validated, see Gap #2 above.
- ✅ **OSS-safe-by-default workflow (v0.8.0)**: new `.githooks/commit-msg` scans the commit message text for owner-specific PII (mirrors `sanitize-lint --staged-only`); `docs/oss-safe-coding.md` documents the three layers of enforcement (file content / commit message / author email); local repo `.git/config` now uses `PiOS Contributors <noreply@pios-ai.org>` so new commits get the project identity automatically — no more orphan-squash author rewriting.
- ✅ **TTS streaming for long replies (v0.8.1)**: `backend/qwen-tts.js`'s `speak()` accepts an `onChunk` callback. When provided, each chunk's WAV is emitted in-order as soon as synthesis completes. `main/ipc-handlers/voice.js` wires the callback to `voice:tts:chunk` IPC events; renderer's `AudioQueue` enqueues each chunk as it arrives. First-byte latency for a multi-chunk reply drops from `N × per-chunk-time` to `1 × per-chunk-time`. Resolves issue #8.
- ✅ **auth-check.sh openclaw notify guard (v0.8.1)**: `backend/tools/auth-check.sh` notify section was missing the `command -v openclaw >/dev/null 2>&1` guard that the `update_engine_status` section already had. Users without openclaw installed received a CRITICAL "weixin channel not OK" notification every hour; with the guard, openclaw is treated as N/A when not installed. Resolves issue #9.
- ✅ **qwen-tts chunk-serial hotfix (v0.8.2)**: long-text synthesis previously used `Promise.all` to fan out N concurrent HTTP requests to the qwen-voice service. mlx-audio's Qwen3-TTS model is not request-isolated — concurrent requests sharing the model state triggered `ValueError: [broadcast_shapes] (1,1,265,265) vs (1,8,265,530)` and a `libmlx` Metal `Device::end_encoding` NULL-pointer deref → SIGSEGV. launchd auto-restarted, but heavy TTS traffic produced 4-5 crashes per hour. Fixed by serializing chunks within `speak()`: total wall time grows from `max(per-chunk)` to `N × per-chunk`, but streaming mode (`onChunk` provided) keeps first-byte latency at `1 × per-chunk`, and audio playback time per chunk is typically ≥ synthesis time so the user perceives no slowdown. Buffer mode also synthesizes serially now.
