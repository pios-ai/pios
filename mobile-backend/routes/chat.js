...
function sendShellHTML(req, res) {
  const html = fs.readFileSync(path.join(__dirname, '..', 'views', 'shell.html'), 'utf-8');
  res.writeHead(200, {
...
function dispatch(req, res, parsedUrl) {
  ...
  if (pathname === '/m/' || pathname === '/m') {
    if (req.method === 'GET') return sendShellHTML(req, res);
  }
