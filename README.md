...
>
> 当前版本：v0.7.9 · macOS arm64 · 求生期（不是成熟产品）
...
PiOS 在求生期。以下 9 条是**当前已知差距**（从 [ARCHITECTURE.md §9](ARCHITECTURE.md#9-known-gaps-current--target) 搬）：

| # | 差距 | 影响 |
|---|---|---|
| 1 | PiOS.app 还不嵌 scheduler | 依赖外部 cron，无法独立分发 |
| 2 | Core agent prompts 硬编码用户名 / 路径 | 换用户要改 prompt |
| 3 | Plugin prompts 硬编码 ASIN / 健康参数 | 用户配置要挪到 config 文件 |
| 4 | Plugins 不注册 data sources / hooks | triage 硬编码 plugin 逻辑 |
| 5 | Product code 还在 user Vault 里 | 产品/用户数据未解耦 |
| 6 | `pios-tick.sh` 存在 3 份拷贝 | 同步发散风险 |
| 7 | 命名不一致（pios / pi-browser / PiOS / PiBrowser） | 体验不统一 |
| 8 | `main.js` 5283 行单文件 | 维护难 |
| 9 | 无自动化测试 | 改动不敢大 |

**现在能用**：owner 自己每天在用，15+ agent + 9 plugin + 5 机器拓扑 + 7 pipeline 数据流都在跑。**现在还不能**：一键分发给非技术用户（DMG 没签名/公证；setup wizard 需要用户预装 Claude Code CLI）。
...
*v0.7.9 · 2026-04-28 更新 · macOS arm64 · 求生期开源工程，不保证向后兼容。版本演进见 CHANGELOG.md。*
