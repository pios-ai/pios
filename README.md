...
**当前 v0.7.14 已知差距**（详见 [ARCHITECTURE.md §9](ARCHITECTURE.md#9-known-gaps-current--target)）：
...
- ✅ **测试基建 (v0.7.14)**:从自制 13-用例 smoke 升级到 `node --test` 框架 + 108 用例 / 22 文件按模块拆,coverage backend pure / lib / main (electron mock) / renderer/lib (UMD + jsdom);GitHub Actions CI matrix node 20+22 跑 sanitize-lint + unit + integration + coverage,pre-commit 跑 unit 子集 ~3s;新模块默认带 sibling `*.test.js`,约定见 `test/CONVENTIONS.md`
...
*v0.7.14 · 2026-04-29 更新 · macOS arm64 · 不保证向后兼容。版本演进见 CHANGELOG.md。*
