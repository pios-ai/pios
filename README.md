...
**当前 v0.8.4 已知差距**（详见 [ARCHITECTURE.md §9](ARCHITECTURE.md#9-known-gaps-current--target)）：
...
- ✅ **`main.js` 7469 → 497 (-93.3%) + 26 模块 ≤800 (v0.7.14)**:单 entry 退化为 thin shell，逻辑全部下放到 `main/` 26 个职责单一模块;HTTP API 拆成 shell + 5 个 sub-handler;新加 IPC handler 5 步流程见 `main/README.md`
...
- ✅ **OSS-safe-by-default 工作流 (v0.8.0)**:`.githooks/commit-msg` 扫 PII;`docs/oss-safe-coding.md` 写明三道闸门(file content / commit message / author email);仓库 local git config 设 `noreply@pios-ai.org` —— 新 commit 自动盖项目身份,不再每次 release 靠 orphan-squash 改 author
- ✅ **TTS 长文本流式播放 (v0.8.1)**:`backend/qwen-tts.js:speak()` 加 `onChunk` 回调;长文本不再 `Promise.all` 等所有 chunk 才返回 — main 端按 idx 顺序通过 `voice:tts:chunk` 事件实时推到 renderer `audioQueue`,首字延迟从 `N*chunk` 降到 `1*chunk`(issue #8)
- ✅ **auth-check 通知误报 (v0.8.1)**:未安装 openclaw 的用户每小时收 CRITICAL「weixin channel not OK」— `backend/tools/auth-check.sh` 通知段加 `command -v openclaw` 守卫,跟 update_engine_status 段语义对齐(issue #9)
- ✅ **TTS chunk 串行 hotfix (v0.8.2)**:`backend/qwen-tts.js:speak()` 内部 chunk 改串行,不再 `Promise.all` 并发 — mlx-audio 模型不是 thread-safe,并发请求触发 `broadcast_shapes` mismatch + libmlx Metal::Device::end_encoding NULL deref → SIGSEGV → launchd 反复重启 qwen-voice。代价:总合成时间 `max(per-chunk)` → `N*per-chunk`,但流式模式下首字延迟仍是 `1*per-chunk`(audio 时长通常 ≥ 合成时长,用户无感)
- ✅ **TTS 真流式端到端 (v0.8.4)**:`_streamPCM` 用上服务端 chunked transfer,每段 PCM 立即包 mini-WAV forward 给 renderer audioQueue。第一字节 6s → ~500ms。renderer audioQueue 改 `AudioContext.currentTime` sample-accurate 调度,修段间"电报咔嗒声"。
- ✅ **installer 升级覆盖 system tools (v0.8.4)**:hash 比对 + 备份 `.bak.<timestamp>` + 覆盖,修升级用户永远停留在旧 `Pi/Tools/*.sh` 的问题(#9 fix 真到达升级用户)。
...
*v0.8.4 · 2026-04-29 更新 · macOS arm64 · 不保证向后兼容。版本演进见 CHANGELOG.md。*
