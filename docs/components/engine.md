...
**Source of truth**: `~/<vault>/Pi/Tools/pios-tick.sh` (Vault). Edit there, commit, Syncthing propagates to all machines.

**Two runtime locations** — Vault is canonical, bundle is a one-time bootstrap source:

| Location | Purpose | Who reads it | Synced via |
|---|---|---|---|
| `~/<vault>/Pi/Tools/pios-tick.sh` (Vault) | **Canonical runtime** — both `.app` form and standalone-CLI form read this | `main/scheduler.js` internal scheduler / user cron | `git commit` + Syncthing across machines |
| `<PiOS.app>/Contents/Resources/app.asar.unpacked/backend/tools/pios-tick.sh` | **Bootstrap-only** — copied to Vault on first install (`pios-installer.js` step 13b), never touched again | `pios-installer.js` (first install only) | `npm run prebuild:*` rsyncs Vault → `backend/tools/` before each build, then electron-builder packages it |
| `Projects/pios/backend/tools/pios-tick.sh` (repo) | **Build input only** — auto-synced from Vault via `prebuild` script; do not hand-edit | `electron-builder` at build time | `npm run sync-vault-tools` (run automatically by `prebuild` / `prebuild:dir` / `prebuild:dmg`) |

**Vault as SSoT (since 2026-04-29 plan c)**:

- `.app` form's `main/scheduler.js` reads **only Vault path** (`$VAULT_ROOT/Pi/Tools/pios-tick.sh`). dev mode (`npm start` without packaging) falls back to `__dirname/backend/tools/`.
- After first install, all script updates flow via Vault: edit `Pi/Tools/*.sh` → commit → Syncthing/git propagates → `.app` picks up next tick (no rebuild required).
- `install-app.sh` no longer syncs bundle → vault. The 2026-04-28 "drift control read bundle only" rule is reverted (it caused 21 commits to be silently overwritten by stale bundle on each install — see `verify-pios-tools-regression-fix-2026-04-29` card).
- Build process pre-step (`npm run prebuild:dir` etc) runs `rsync -a ../../Pi/Tools/ backend/tools/` so the bundle's bootstrap snapshot reflects the latest committed scripts. Forget to rebuild = next-installed user gets older scripts (Syncthing later catches them up); but no impact on existing installs.

**Trigger**: `PiOS.app` internal `setInterval` every 60s (default), or `* * * * *` cron for standalone-CLI form.
