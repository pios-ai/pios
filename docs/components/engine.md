...
|---|---|---|---|
| `~/<vault>/Pi/Tools/pios-tick.sh` (Vault) | **Canonical runtime** — both `.app` form and standalone-CLI form read this | `main.js` internal scheduler / user cron | `git commit` + Syncthing across machines |
...
- `.app` form's `main.js` scheduler reads **only Vault path** (`$VAULT_ROOT/Pi/Tools/pios-tick.sh`). dev mode (`npm start` without packaging) falls back to `__dirname/backend/tools/`.
