...
|-------|------|------|
| `{owner}` | 用户短名（ID） | `owner` |
| `{vault}` | Vault 根绝对路径 | `/Users/abe/PiOS-Vault` |
...
owner: owner
display_names:
  wechat: owner郭         # → {wechat_name}
  english: owner          # → {english_name}   (future-proof 示例)
  family: 爸爸          # → {family_name}    (future-proof 示例)
```

⚠️ **铁律：禁止 `{owner}` + 字面量拼接**。Tony 不会叫 `Tony郭`，prompt 里 `{owner}郭` 这种写法等于硬编码 owner。所有 per-user 的整串显示名必须整体进 `display_names:`，prompt 里用 `{<key>_name}` 单一占位符。
...
2. **参数化替换**：
   ...
   - `owner郭`（整串微信名） → `{wechat_name}`（对应 `display_names.wechat: "owner郭"`）；**不要**拆成 `{owner}郭`，Tony 不会叫 Tony郭
   - `郭奕诗`（英文名/护照名/其他整串 per-user 字符串） → 新增 `display_names.<key>` 条目 + `{<key>_name}` 占位符
