...
>
> 权威来源：`Pi/Memory/project_pios_goals.md`（四层目标）、`Pi/Config/alignment.md`（四种力量）、`owner/Profile/Abe_Profile.md`（创建者画像）。
...
PiOS 当前是 single-owner。Vault 里很多东西写死了一个 owner（`{owner}郭` 之类），多人共用同一个 Vault 还没支持。
...
对应到 owner 自己的三本能观（见 [`owner/Profile/Abe_Profile.md`](../../../owner/Profile/Abe_Profile.md) §价值观）：求生、好奇、慈悲。
...
> PiOS 适合：已经重度用 AI、不怕脏手、想要一个在自己电脑上 7×24 帮你干活的"有记忆的同事"、能接受产品还在求生期的人。
