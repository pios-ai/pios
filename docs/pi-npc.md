...
新皮肤只改 **renderer 的 CSS/DOM** 和 **一行 main/bubble-npc-tray.js 注册**，信号层（pi-pulse）、IPC、pose 词表、交互层全部不动。
...
1. `renderer/bubble.html`
   ...
   - 实现 11 个 pose 的视觉差分
2. `main/bubble-npc-tray.js`：`NPC_SKINS` 数组里 push 一项 `{ id: '<newid>', label: '<显示名>' }`
...
- [Projects/pios/main/bubble-npc-tray.js](../main/bubble-npc-tray.js) — NPC 系统 + Tray + 浮动语音气泡 整套（2026-04-29 从 main.js 拆出）；`tts:playback-state` IPC handler 在此
...
#    - main/bubble-npc-tray.js:  整个 NPC 系统（2026-04-29 从 main.js 拆出）
...
- **main/bubble-npc-tray.js 里 `NPC_SIZE_ON` / `NPC_SIZE_OFF` / `NPC_STATE_FILE` 必须在 `createBubbleWindow` 函数定义之前声明**：bubble 窗口在启动时立即创建并读这些常量，声明晚于引用会触发 TDZ，orb + Patrick 双双不显示
