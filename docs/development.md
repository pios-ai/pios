...
| NPC / 派大星 (bubble.html + pi-pulse.js + main/bubble-npc-tray.js) | 气泡窗口显示、pose 动画、toast/speak 气泡、点击收气泡、TTS 打断 | 读 [pi-npc.md](pi-npc.md)；renderer 改动警惕 TDZ（`const` 声明必须在引用之前，否则整个 script block throw，IPC handler 全部失效） |
