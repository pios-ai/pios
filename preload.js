...
contextBridge.exposeInMainWorld('pi', {
  ...
  sessionLoad: (id, opts) => ipcRenderer.invoke('sessions:load', id, opts || {}),
  sessionSave: (session, opts) => ipcRenderer.invoke('sessions:save', session, opts || {}),
  ...
  // TTS（Codex 模式 + 长文本流式）
  // voiceTTS 仍是 invoke，但长文本时 main 不再返回 merged buffer ——
  // 每段合成完毕走 onVoiceTTSChunk 事件实时推到 renderer audioQueue。
  // invoke 本身仍 await 到末段合成完才 resolve，调用方拿到的是 null（流式）或最后一段 buffer（短文本）。
  voiceTTS: (text, preset) => ipcRenderer.invoke('voice:tts', text, preset),
  onVoiceTTSChunk: (callback) => ipcRenderer.on('voice:tts:chunk', (_, buf) => callback(buf)),
