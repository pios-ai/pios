...
slug: personal-ai-operating-system-architecture
target_keywords:
...
- **Scheduler**: Cron-like job scheduling for pipelines and agents. We use APScheduler, but launchd (macOS) or systemd timers (Linux) work equally well for simple setups.
...
| source-chatgpt | ChatGPT web (browser automation) | AI conversation summaries |
| source-calendar | CalDAV / Apple Calendar | Daily schedule |

**Design principle: Each plugin is independent.** If the WeChat plugin crashes, the health plugin still runs. Plugins don't know about each other. They only know how to produce documents.
...
```yaml
---
id: doc_a1b2c3d4        # Auto-generated UUID
title: "Daily Health Report"
type: report             # report | digest | note | task | journal
...
Or, use PiOS as a starting point — it implements all of this as an installable, configurable system.

## Resources

- **PiOS Repository**: [github.com/pios-ai/pios](https://github.com/pios-ai/pios)
- **Architecture Document**: `docs/ARCHITECTURE.md` in the PiOS repo
- **Plugin Development Guide**: `docs/PLUGIN_GUIDE.md` in the PiOS repo
