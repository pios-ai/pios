...
function install(options) {
  const {
    owner_name = 'User',
    // 注意：macOS HFS+ 默认 case-insensitive，~/PiOS 跟 ~/pios（git clone 默认目录名）
    // 是同一个目录。2026-04-29 fresh 装机用户 vault 跟源码 clone 撞在一起，源码文件
    // 渗到 vault 让 step 6 lookup 错乱（Pi/Plugins/core 找到 dev 残留就跳）。
    // default 改成 ~/PiOS-Vault 跟 ~/pios 区分开。
    vault_root = path.join(process.env.HOME, 'PiOS-Vault'),
  ...
  const manifest = {
    ...
    agents: {
      ...
      'pipeline': {
        ...
        host: hostname,
        status: 'active',
        // 全部 pipeline task 默认 disabled —— 用户在 PiBrowser Resources/Team 面板按需打开。
        // 首装阶段不该自动跑 pipeline：cron 触发会消耗 token、写 vault、发通知，
        // 但用户这时还没决定要用哪些、也没配置好凭据。
        tasks: {
          'daily-ai-diary': {
    ...
    sense: {
      // 全部 sense.pipelines.*.enabled 默认 false，跟 agents.pipeline.tasks.*.enabled
      // 同步（issue #4 fix 当时只改了 agents.pipeline.tasks 那一半，留了 sense.pipelines
      // 这一半 enabled:true，导致 Tasks tab 显示 disabled 但 Team/Pi/Pipeline UI 显示
      // enabled，两边不一致——2026-04-29 fresh 装机用户实测踩到）。
      // 用户在 PiBrowser Resources / Team 面板按需 toggle 开。
      pipelines: {
        'ai-diary': {
          built_in: true,
...
`;
  ...
  //    优先读 app bundle 内的 backend/plugins/core；dev 路径仅作 fallback。
  //
  // 历史 bug：2026-04-29 新机器装上后 Pi/Agents/pi 全空、Tasks tab 空。根因是
  // 这个 priority 反了——dev 路径 path.join(__dirname, 'Pi', 'Plugins', 'core')
  // 在 fresh clone 里偶尔残留一个孤零零的 SOUL.md（来自历史 commit 或 dev test
  // 环境），fs.existsSync(目录) 返回 true，但目录里没 agents/pi/SOUL.md 等关键
  // 子文件，step 6 全部 fs.existsSync(src) → false → 静默跳过。
  //
  // 修法：(a) BUNDLED_CORE_DIR 放第一优先，(b) 不只 check 目录存在，要 check
  // 关键子文件 agents/pi/SOUL.md 存在。
  const _hasCorePayload = (p) => fs.existsSync(path.join(p, 'agents', 'pi', 'SOUL.md'));
  const corePluginSrc = [
    BUNDLED_CORE_DIR,                                  // 打包后真实 bundle，永远 ship
    path.join(__dirname, 'Pi', 'Plugins', 'core'),     // dev 路径，仅在 source 跑 npm start 时存在
  ].find(p => fs.existsSync(p) && _hasCorePayload(p));

  if (corePluginSrc) {
    // 复制 Kernel (pi) + Pipeline + Radar Agent SOUL 到 runtime 实际读取的 Pi/Agents/
    // maintenance / sense-maker 是 pi 下面的 task 不是独立 agent，不在这里
    // radar 当前 0 task（用户自己加），SOUL 还是 ship 让 Pi > Radar tab 有 agent 上下文
    const standardAgents = ['pi', 'pipeline', 'radar'];
...
`;
  ...
  // 2026-04-30 issue #9 followup：之前 if (fs.existsSync(dest)) continue 的"用户改过不覆盖"
  // 策略让升级用户永远停留在旧版 system tool（如 auth-check.sh 的 openclaw 误报 fix
  // ship 在 DMG 里但 installer 不覆盖 → 仍跑旧版本）。改成：hash 比对，不同则覆盖 +
  // 保留 .bak.<timestamp>，用户真改过可以从 .bak 回滚。
  ...
  const _copyToolsRec = (srcDir, destDir) => {
    ...
    for (const name of fs.readdirSync(srcDir)) {
      ...
      if (fs.existsSync(dest)) {
        // hash 一样 → 跳过（无需 IO）；不一样 → 备份 + 覆盖
        if (_hashFile(src) === _hashFile(dest)) continue;
        try {
