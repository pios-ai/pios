...
**Theme: streaming TTS audio fidelity hotfix**

### Fixed
- **`renderer/app.js` `AudioQueue` choppy playback during streaming TTS**: v0.8.4 shipped sample-accurate scheduling but kept the default 48 kHz `AudioContext`. mlx-audio outputs 24 kHz mono PCM, so Web Audio resampled each mini-WAV independently at chunk boundaries — the per-segment resample produced phase discontinuities that sounded like "断断续续" (intermittent / choppy) on long replies. Two changes: (1) construct `AudioContext({ sampleRate: 24000 })` so the source rate matches the output context end-to-end with no per-chunk resample, (2) serialise `decodeAudioData` calls via a promise chain so concurrent `enqueue()` invocations can't reorder buffers across the prebuffer boundary. Verified via `/tmp/web-audio-sim.html` single-variable test (A=48k+parallel choppy, B=24k+serial smooth, C=24k+serial+grouped smooth — B is the minimal fix) plus end-user ear test on the same long reply that was choppy in v0.8.4.

---

## [0.8.4] — 2026-04-30

**Theme: real TTS streaming + installer auto-update + CI fix**

### Added
- **TTS true streaming end-to-end**: `backend/qwen-tts.js` adds `_streamPCM(text, voice, instruct, timeout, onMiniWav)` that uses qwen-voice's `/api/tts/stream` chunked transfer properly — server-side mlx-audio is autoregressive token-level (`stream=True`), each chunked PCM segment is forwarded immediately as a self-contained mini-WAV (44-byte RIFF header + ~0.3s of 24 kHz mono PCM). When `tts.speak()` receives an `onChunk` callback, it goes down the streaming path instead of accumulating into one buffer. First-byte latency drops from ~6 s to ~500 ms — measured via the TTS endpoint test on local mac with a multi-chunk reply. Voice-filter (`applyVoiceFilter`) is intentionally bypassed in streaming mode (decision: bake the chorus/echo magnetic effect into ref-audio at preprocess time instead of post-processing each output, separate one-time setup task).
- **`renderer/app.js` `AudioQueue` rewritten for sample-accurate scheduling**: chunks are now scheduled on the AudioContext's high-precision clock (`src.start(max(now, lastEnd))`) instead of chained via `onended`. The previous chain-by-callback pattern introduced a ~50 ms decode/dispatch gap between consecutive mini-WAVs that sounded like telegraph clicks during streaming. With clock-based scheduling, consecutive chunks play seamlessly.

### Fixed
- **`pios-installer.js` upgrade path silently kept old `Pi/Tools/*.sh`**: the installer used `if (fs.existsSync(dest)) continue` to "respect user edits," but for upgrade installs this meant the bundled fresh `auth-check.sh` from a new DMG was skipped — leaving the user with whatever version was first installed (e.g., the openclaw-false-positive bug was fixed in v0.8.3 source but never reached upgraders' vaults). Replaced with hash-based update: same hash → skip; different hash → backup current to `.bak.<timestamp>` and overwrite. New users see no change; upgrade users finally get fixes that ship in `Pi/Tools/`.
- **`.github/workflows/mobile-m1.yml` Playwright env mismatch**: the workflow set `MOBILE_API_TOKEN=ci-test-token` for both server and Playwright steps, but `tests/mobile-responsive.spec.js` actually reads `PI_API_TOKEN` (with fallback `'test-token-m1'`). Tests sent the fallback token, server expected the CI token, all `/mobile/hello` requests 401'd, downstream assertions failed. Fix: also set `PI_API_TOKEN: ci-test-token` in the Playwright run env.
