...
function _renderTaskOutcomeBanner(session) {
  ...
  if (status === 'failed') {
    ...
    // ⚠ 2026-04-29 文案修订：旧文案"OOM/SIGKILL/hang"会诱导人手动 kill。
    // heartbeat loop 偶尔 break 但 adapter+claude 多数仍在工作；EXIT trap 会延迟 finalize。
    const bits = ['心跳停滞'];
    bits.push('adapter 通常仍在工作或正在自然结束，会自动 finalize 成 success/failed');
    bits.push('仅当 >10min 仍未 finalize 且 PID 已不存在时才考虑人工介入');
...
async function createSession() {
  ...
  threadId = null;
  await window.pi.sessionSave(currentSession, { setActive: true });
...
async function _loadSessionImpl(id) {
  let session = await window.pi.sessionLoad(id, { setActive: true });
  if (!session) return false; // 不存在的 session，调用方自己处理
  // 又一道闸：如果这次 load 在 sessionLoad 期间用户已经又切走了，不要污染 UI
  if (_loadSessionLatestRequestedId !== id) return false;
...
async function saveCurrentSession() {
  ...
  // threadId is Codex-specific. Do not let Claude/GPT turns overwrite the
  // saved Codex thread when the user switches engines inside one PiBrowser
  // session and later comes back to Codex.
  if (currentEngine === 'codex') currentSession.threadId = threadId || null;
  await window.pi.sessionSave(currentSession, { setActive: true });
  // 防抖刷新左侧边栏
...
function _buildSessionItem(s) {
  ...
  // ⚠ 2026-04-29 文案修订 + 自定义 tooltip：旧文案"进程失联（adapter 心跳已停）"会诱导人手动 kill。
  // 实情是 heartbeat loop 偶尔 break 但 adapter+claude 多数仍在工作；强 kill 会丢任务进度。
  // 用 data-pi-tooltip 替代 title，避免 macOS 原生 tooltip 的 1.5s 延迟（CSS 在 styles.css）。
  // 详见 Cards/active/verify-zombie-tooltip-rename-2026-04-29.md
  const pillHtml = (s.runStatus === 'zombie')
    ? '<span class="session-status-pill zombie" data-pi-tooltip="心跳停滞 90s+。多数情况 adapter 仍在工作或正在自然结束，会自动 finalize；不要手动 kill"></span>'
    : (s.runStatus === 'timeout')
...
const ENGINE_CYCLE = ['auto', 'gpt', 'codex', 'claude', 'clean'];
...
function cycleEngine() {
  const idx = ENGINE_CYCLE.indexOf(currentEngine);
  setEngine(ENGINE_CYCLE[(idx + 1) % ENGINE_CYCLE.length]);
}
...
window.pi.onSessionBusEvent((payload) => {
  ...
  if (payload.type === 'session-meta') {
    const tid = payload.codexThreadId || payload.threadId || null;
    if (!tid) return;
    if (currentSession?.id === sid) {
      _setVisibleThreadIdForSession(sid, tid);
      saveCurrentSession();
    } else {
      _queueSessionMetaPatch(sid, { threadId: tid });
    }
    return;
  }
  ...
  if (payload.type === 'tool') {
    ...
    // 同时把错误消息渲染到 chat 气泡里，避免用户只看 activity panel 看不到（2026-04-29
    // fresh 装机 Claude 未登录时 Pi 看着像"无响应"，实际是 Not logged in 错被吞了）。
    // 当前对话面板才推 chat 气泡；后台 task session 仍走 failV2Activity 不动。
    const knownOrigin = _getKnownSessionOrigin(sid);
    if (knownOrigin !== 'task' && currentSession?.id === sid) {
      try { addMessage(chatMessages, 'ai', '⚠️ ' + (payload.content || '执行失败')); } catch {}
    }
    if (knownOrigin === 'task') {
...
function _stripSayTags(text) {
  return String(text || '').replace(/<say(?:\s+voice="\w+")?\s*>[\s\S]*?<\/say>/g, '').trim();
}

// ── 打通 Codex/Claude/GPT 上下文 ──
function injectCrossEngineContext(message, session = currentSession, engine = currentEngine) {
  if (!session || !Array.isArray(session.messages) || session.messages.length === 0) return message;
  if (String(message || '').startsWith('[同一 PiBrowser 会话历史]')) return message;
  const msgs = session.messages.filter(m => m && m.silent !== true && m.engine !== 'silent');
  const hasOther = msgs.some(m => m.engine && m.engine !== engine);
  if (!hasOther) return message;
  // 引擎切换时 CLI thread 不共享；把同一 PiBrowser session 的近期历史显式接力给新引擎。
  // TNY / Call Pi 的首条任务很长，不能只截 300 字，否则 Codex 会丢掉卡片路径和 owner 指令。
  const recent = msgs.slice(-12);
  const engineLabel = { codex: 'Codex', gpt: 'GPT', claude: 'Claude' };
  let remaining = 18000;
  const lines = recent.map(m => {
    const eng = engineLabel[m.engine || engine] || m.engine || engine;
    const who = m.role === 'user' ? 'User' : `Pi(${eng})`;
    const content = _stripSayTags(m.content);
    if (!content) return null;
    if (remaining <= 0) return null;
    const cap = m.role === 'user' ? 5000 : 2200;
    const take = Math.max(0, Math.min(cap, remaining));
    remaining -= take;
    const clipped = content.length > take ? content.substring(0, take) + '\n...[已截断]' : content;
    return `${who}:\n${clipped}`;
  }).filter(Boolean);
  if (lines.length === 0) return message;
  const others = [...new Set(msgs.filter(m => m.engine && m.engine !== engine).map(m => engineLabel[m.engine] || m.engine))];
  return `[同一 PiBrowser 会话历史]\n之前的交互包含 ${others.join('/')}。当前已切换到 ${engineLabel[engine] || engine}，请把下面历史当作同一个会话继续，不要说自己看不到上文。\n\n${lines.join('\n\n')}\n\n[当前问题]\n${message}`;
}
...
async function sendMessage(input, messagesContainer, options = {}) {
  ...
  // 打通上下文。用本轮 sendSid 对应的目标 session，而不是盲用当前屏幕 session；
  // 否则排队消息或后台 session 续跑时会把别的会话历史注入给新引擎。
  const contextSession = currentSession?.id === sendSid
    ? currentSession
    : ((await window.pi.sessionLoad(sendSid)) || currentSession);
  const contextEngine = requestedEngine === 'auto' ? 'gpt' : requestedEngine;
  fullMessage = injectCrossEngineContext(fullMessage, contextSession, contextEngine);
  ...
  sidebarPlusMenu.querySelectorAll('.plus-item').forEach(item => {
    item.addEventListener('click', async () => {
      ...
      if (action === 'attach') {
        ...
        window.pi.resetGPT();
        enterWelcomeState();
        await window.pi.sessionSave(currentSession, { setActive: true, allowMessageTruncate: true });
        renderSessionList();
      }
    });
  });
...
class AudioQueue {
  constructor() {
    this._ctx = null;
    this._activeSrcs = new Set();   // currently scheduled BufferSourceNodes
    this._nextStartTime = 0;        // AudioContext.currentTime when next segment should begin
    this._playing = false;
    this._onIdle = null;
    this._idleTimer = null;
    // Pre-buffer queue (mlx-audio 合成不稳定 realtime — 累积 N 段音频 / N 秒音频
    // 再开播，给合成端留余量。期间陆续到达的段先解码 + 入队，不调度。
    // 第一段到达就触发 _maybeStartPlayback —— 满足任一条件即 flush 队列开播。
    // 2026-04-30：streaming_interval=1.0 + 合成 ~1.2-1.5x realtime → 不预缓冲会出现
    // 段间 0.5s 静默累积。预缓冲 3 段 / 1.5s 后开播，进入稳态后通常追平。
    this._prebuffer = [];
    this._prebuffered = false;
    this._PREBUF_MIN_SEGMENTS = 3;       // 至少 3 段
    this._PREBUF_MIN_DURATION = 1.5;     // 或累计 1.5s 音频，先到任一即开播
    this._PREBUF_TIMEOUT_MS = 1500;      // 或上一段到达超过这个时间没新段，强制开播
    this._prebufTimer = null;
    // 串行化 decodeAudioData：多次 enqueue 不阻塞调用方，但 decode/调度顺序保证 FIFO。
    // 2026-04-30：并行 decode 在 sampleRate 不匹配时会出现段间乱序；显式串行 + 24k ctx 双保险。
    this._decodeChain = Promise.resolve();
  }

  async _ensureCtx() {
    // 显式 sampleRate 匹配 mlx-audio 输出（24kHz）。默认 48kHz 会让 Web Audio 在边界
    // 把每段独立 resample，相位不连续 → 段间「滴答 / 断断续续」。
    // 2026-04-30：file:// 模拟验证 A=48k+并行 断；B=24k+串行 连续；C=24k+串行+大块 连续 → B 是最小修法。
  ...
  _scheduleBuffer(decoded) {
    ...
    src.start(startAt);
    this._nextStartTime = startAt + decoded.duration;
    this._activeSrcs.add(src);
    if (!this._playing) {
      this._playing = true;
      try { window.pi?.ttsPlaybackState?.(true); } catch {}
    }
    if (this._idleTimer) { clearTimeout(this._idleTimer); this._idleTimer = null; }
    src.onended = () => {
      this._activeSrcs.delete(src);
      if (this._activeSrcs.size === 0 && ctx.currentTime >= this._nextStartTime - 0.01) {
        if (this._idleTimer) clearTimeout(this._idleTimer);
        this._idleTimer = setTimeout(() => {
          if (this._activeSrcs.size === 0 && this._prebuffer.length === 0) {
            this._playing = false;
            this._nextStartTime = 0;
            this._prebuffered = false;
            try { window.pi?.ttsPlaybackState?.(false); } catch {}
  ...
  _flushPrebuffer() {
    if (this._prebufTimer) { clearTimeout(this._prebufTimer); this._prebufTimer = null; }
    while (this._prebuffer.length > 0) {
      this._scheduleBuffer(this._prebuffer.shift());
    }
    this._prebuffered = true;
  }

  /**
   * Schedule each chunk on AudioContext's high-precision clock + pre-buffer
   * the first N chunks (mlx-audio 合成不稳定 realtime,直接 src.start 会出现段间
   * 静默累积「断断续续」).
   */
  async enqueue(arrayBuf) {
    if (!ttsEnabled.checked) return;
    // 串行化：链上每段顺序 decode + 入队 / 调度，防 enqueue 并发交错。
    this._decodeChain = this._decodeChain.then(async () => {
      try {
        ...
        if (this._prebuffered) {
          this._scheduleBuffer(decoded);
          return;
        }

        // 预缓冲阶段：累积；满足条件 flush
        this._prebuffer.push(decoded);
        const totalDur = this._prebuffer.reduce((s, b) => s + b.duration, 0);

        if (this._prebuffer.length >= this._PREBUF_MIN_SEGMENTS || totalDur >= this._PREBUF_MIN_DURATION) {
          this._flushPrebuffer();
          return;
        }

        // 重置 timeout：上一段到达后 N ms 没新段 → 不再等，flush
        if (this._prebufTimer) clearTimeout(this._prebufTimer);
        this._prebufTimer = setTimeout(() => {
          if (this._prebuffer.length > 0) this._flushPrebuffer();
        }, this._PREBUF_TIMEOUT_MS);
  ...
  interrupt() {
    ...
    this._activeSrcs.clear();
    this._nextStartTime = 0;
    this._prebuffer = [];
    this._prebuffered = false;
    // 中断也重置 decode 链——上一段未完成的 decode 别再触发调度
    this._decodeChain = Promise.resolve();
    if (this._prebufTimer) { clearTimeout(this._prebufTimer); this._prebufTimer = null; }
    if (this._idleTimer) { clearTimeout(this._idleTimer); this._idleTimer = null; }
    if (this._playing) {
...
window.pi.onNotifySpeak(async (text) => {
  ...
  try {
    // chunks 走 voice:tts:chunk 事件流自动 enqueue（v0.8.1 起）；这里只 await 完成，不再
    // enqueue invoke 返回值——否则跟事件流的 last chunk 撞，短通知（单 chunk）会念两次
    await window.pi.voiceTTS(text, 'calm');
    try { window.pi.debugTrace('renderer', 'voiceTTS done (chunks streamed via voice:tts:chunk)'); } catch {}
  } catch (e) {
...
// ── TTS: 流式分段播放（通过 AudioQueue）──
// 修 issue #8：长文本不再等所有 chunk 合成完才播 —
// main 端 voice:tts:chunk 事件按 idx 顺序逐段推过来，audioQueue.enqueue 顺序播
window.pi.onVoiceTTSChunk?.((buf) => {
  if (buf && buf.byteLength > 100) audioQueue.enqueue(buf);
});
...
async function playTTS(text, preset) {
  ...
  try {
    // chunks 走 voice:tts:chunk 事件实时 enqueue；invoke 仅 await 到末段完成
    await window.pi.voiceTTS(text, preset);
...
const COMMANDS = [
  { name: '切换引擎', key: 'Cmd+E', action: cycleEngine },
...
(async function checkPiOSSetup() {
  ...
  document.getElementById('setup-install').addEventListener('click', async () => {
    ...
    // 默认装的 plugin（setup wizard 透传给 installer 的 install({plugins})）。
    // 加 'voice' = 让 fresh 装机用户在 Resources 直接看到 NPC 语音引擎激活按钮，
    // 不用先去翻 docs/setup-qwen-voice.md 才知道 voice 是个可装的东西（issue #5）。
    const plugins = ['vault', 'shell', 'web-search', 'browser', 'wechat', 'voice'];
