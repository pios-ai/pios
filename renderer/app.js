...
class AudioQueue {
  ...
  /**
   * Schedule each chunk on AudioContext's high-precision clock so consecutive
   * mini-WAVs play seamlessly. Web Audio's BufferSourceNode supports sample-
   * accurate start times — no decode/dispatch latency between chunks, so no
   * "telegraph click" at chunk boundaries (2026-04-30 fix for 0.3s mini-WAV
   * streaming from qwen-voice).
   */
  async enqueue(arrayBuf) {
    ...
    try {
      ...
      // Schedule: max(now, where last chunk ends). 'now' wins on first chunk
      // or after a long idle; 'last end' wins for back-to-back streaming chunks.
      ...
      src.onended = () => {
        ...
        // If nothing scheduled past 'now', we've drained — but wait a beat to
        // avoid bouncing isPlaying off/on for each chunk during steady streaming.
        if (this._activeSrcs.size === 0 && ctx.currentTime >= this._nextStartTime - 0.01) {
          if (this._idleTimer) clearTimeout(this._idleTimer);
          this._idleTimer = setTimeout(() => {
            if (this._activeSrcs.size === 0) {
              this._playing = false;
