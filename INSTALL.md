...
This guide covers the current PiOS v0.7.14 macOS install flow for new users.

## Current Scope

- Platform: macOS arm64 (Apple Silicon)
- Preferred public artifact: `PiOS-0.7.14-arm64.dmg` after a verified successful build
...
- Fresh local `npm run build` is currently not stable for DMG output on this host: `electron-builder` can finish `dist/mac-arm64/PiOS.app` but may fail at `hdiutil create` while an older `dist/PiOS-0.7.14-arm64.dmg` still remains in `dist/`.
...
2. If you have a verified DMG, open `PiOS-0.7.14-arm64.dmg` and drag `PiOS.app` into `/Applications`.
3. If you are validating a local build from this host, run `npm run build:dir` and copy `dist/mac-arm64/PiOS.app` into `/Applications` directly instead of trusting a leftover `dist/PiOS-0.7.14-arm64.dmg`.
...
- Current build output:
  ...
  - `dist/PiOS-0.7.14-arm64.dmg` may already exist from an earlier successful build; do not treat its presence alone as proof that the latest build completed.
