...
const HOST_SHORT = require('./backend/lib/host-resolve').resolveHost();
...
/**
 * 原子写入 —— 避免 reader 读到 truncate 中的半写入文件。
 * fs.writeFileSync 默认会先把文件 truncate 到 0 字节再写，
 * 如果 reader 在这几毫秒窗口内读，就会拿到空文件。
 * pios.yaml 被 tick 每分钟读一次，不能忍受这种竞态。
 *
 * 做法：写到 tmp，rename 到目标（同文件系统 rename 是原子的）。
 */
...
function getDailyBriefing() {
  ...
  // Recent runs in last 12h (success + failed + degraded)
  // 之前用 r.status === 'ok' / r.endTime 永远 false —— 实际字段是 status='success'
  // 和 finished_at。这个过滤器静默给回空数组导致 Pi 动态 "最近运行" 空白。
  const allRuns = getRecentRuns(50);
  const TERMINAL = new Set(['success', 'failed', 'degraded']);
  const recentRuns = allRuns
    .filter(r => r && TERMINAL.has(r.status) && r.finished_at && new Date(r.finished_at).getTime() >= twelveHoursAgo)
    .slice(0, 10)
    .map(r => ({ agent: r.agent, taskId: r.taskId || r.plugin_name || r.task, endTime: r.finished_at, duration: r.duration_ms, status: r.status }));
