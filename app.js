...
function addMessage(container, role, text, options = {}) {
  ...
  label.innerHTML = `${role === 'user' ? 'You' : 'Pi'} <span class="msg-time" title="${msgDate.toLocaleString()}">${formatTime(msgDate)}</span>`;
  ...
  if (role === 'ai') {
    ...
    // Footnote click → scroll to source card + navigate
...
async function loadSession(id) {
  ...
  if (session.messages.length === 0) {
    ...
    for (const msg of session.messages) {
      ...
      addMessage(chatMessages, role, msg.content, { injectedPage: msg.injectedPage, attachments: msg.attachments, timestamp: msg.timestamp });
    }
  }
...
function _buildSessionItem(s) {
  ...
  // 状态 pill：running 是脉冲点，unread 是实心点（颜色在 CSS 里accessible colors for colorblind users）
  const pillHtml = (listSt?.running)
    ...
    : (s.runStatus === 'failed')
      ...
      : (s.runStatus === 'degraded')
        ? '<span class="session-status-pill degraded" title="降级完成"></span>'
    : (listSt?.unread)
...
const ENGINE_MODELS = {
  ...
  auto: CLAUDE_MODELS,
  codex: [
    { id: 'gpt-5.4',          label: 'GPT-5.4',       desc: '最强' },
...
function _syncModelBtn() {
  ...
  _allModelBtns.forEach(btn => {
    ...
    btn.title = title;
    btn.classList.toggle('model-mismatch', !!mismatch);
  });
}
...
function setEngine(engine) {
  ...
  [headerEngineBtn, sidebarHeaderEngineBtn].forEach(btn => {
    if (btn) btn.textContent = `Pi ${label} ▾`;
  });
  // 更新下拉选项 active 状态
...
async function sendAgentTask(text, messagesContainer) {
  ...
  try {
    ...
    if (result.error) {
      ...
      addMessage(messagesContainer, 'ai', result.content);
      if (currentSession) {
        currentSession.messages.push({ role: 'ai', content: result.content, engine: 'agent', timestamp: new Date().toISOString() });
        saveCurrentSession();
      }
    }
  } catch (err) {
...
async function _maybeFetchContextWindow(sid) {
  ...
  try {
    ...
    if (data.ok && currentSession?.claudeSessionId === claudeSid) {
      if (data.max) currentSession.contextWindow = data.max;
      currentSession._ctxLastFetchAt = Date.now();
      currentSession._ctxDetailCache = data; // 整个 payload 缓存，点击 modal 时秒出
      const st = _uiStates.get(sid);
      if (st && currentSession?.id === sid) _updateTokenUsageUI(st.tokenUsage);
    }
  } catch (e) {
...
function _getCompactCapabilityForSession(session) {
  if (!session) return { supported: false, reason: 'no-session' };
  if (_isLikelyCodexSession(session)) {
    return {
      supported: false,
      reason: 'codex-unsupported',
      message: 'Codex 会话已接近上下文上限；当前暂不支持自动压缩，且本次没有执行 compact。',
    };
  }
  if (!session.claudeSessionId) return { supported: false, reason: 'no-claude-session' };
  return { supported: true, claudeSid: session.claudeSessionId };
}
...
function _maybeAutoCompact(sid, tokenUsage) {
  ...
  const capability = _getCompactCapabilityForSession(session);
  if (!capability.supported) {
    if (capability.reason === 'codex-unsupported') {
      _showCompactUnavailable(sid, contextFill, win, capability.message);
    }
    return;
  }
  _promptAutoCompact(sid, contextFill, win, capability.claudeSid);
}

function _showCompactUnavailable(sid, contextFill, win, message) {
  const pct = Math.round((contextFill / win) * 100);
  const st = _compactState.get(sid) || {};
  if (st.toastEl) { st.toastEl.remove(); st.toastEl = null; }
  st.lastCompactAt = Date.now();
  st.toastEl = _showCompactToast({
    text: `${message} 当前占用 ${_formatTokenCount(contextFill)} / ${_formatTokenCount(win)}（${pct}%）。`,
    timeout: 10000,
  });
  _compactState.set(sid, st);
}

function _promptAutoCompact(sid, contextFill, win, claudeSid) {
  if (!claudeSid) return;
  const pct = Math.round((contextFill / win) * 100);
...
async function _fireCompact(sid, claudeSid) {
  ...
  try {
    const capability = _getCompactCapabilityForSession(currentSession?.id === sid ? currentSession : null);
    if (!capability.supported) {
      progressToast.remove();
      _showCompactToast({ text: capability.message || '当前会话不支持自动压缩', timeout: 8000 });
      finishWith({});
      return;
    }
...
function _activityStepKind(icon = '', desc = '') {
  const text = `${icon} ${desc}`.toLowerCase();
  if (icon === '⚡' || icon === '⌨️' || text.includes('终端')) return '终端步骤';
  if (icon === '📖' || icon === '📂' || icon === '🔎' || icon === '🔍' || icon === '👆' || icon === '🖼️' || icon === '🌐') return '查看步骤';
  if (icon === '🗺️') return '计划步骤';
  if (icon === '✏️' || icon === '📝') return '修改步骤';
  if (icon === '🐙') return 'GitHub 步骤';
  if (icon === '🧹') return '整理步骤';
  return '执行步骤';
}

function _updateActivityGroupSummary(group, icon = '', desc = '') {
  if (!group) return;
  const nextKind = _activityStepKind(icon, desc);
  const prevKind = group.dataset.kind || '';
  const kind = !prevKind || prevKind === nextKind ? nextKind : 'mixed';
  group.dataset.kind = kind;
  const stepsInGroup = group.querySelectorAll('.exec-step').length;
  const summary = group.querySelector('summary');
  if (!summary) return;
  summary.textContent = kind === 'mixed'
    ? `${stepsInGroup} 个步骤`
    : `${stepsInGroup} 个${kind}`;
}
...
function _clearPendingActivityFinish(st) {
  if (!st) return;
  if (st._pendingFinishTimer) {
    clearTimeout(st._pendingFinishTimer);
    st._pendingFinishTimer = null;
  }
  st._pendingFinishReason = null;
}

function _schedulePendingActivityFinish(sid, delayMs = 700, reason = 'trailing-events') {
  const st = _uiStates.get(sid);
  if (!st) return;
  _clearPendingActivityFinish(st);
  st._pendingFinishReason = reason;
  st._pendingFinishTimer = setTimeout(() => {
    const latest = _uiStates.get(sid);
    if (!latest || latest._pendingFinishReason !== reason) return;
    _clearPendingActivityFinish(latest);
    if (latest.status === 'running' && latest.queuedSends?.length > 0) return;
...
window.pi.onSessionBusEvent((payload) => {
  ...
  // 刀 3: replay 事件 —— 历史已经通过 sessions.json messages 渲染了，跳过
  ...
  if (payload.type === 'usage') {
    ...
    if (raw) {
      ...
      if (raw.model_context_window) sessionMetaPatch.contextWindow = raw.model_context_window;
  ...
  const _handleTaskTextEvent = (txt) => {
    ...
    if (currentSession?.id === sid) {
      addMessage(chatMessages, 'ai', txt);
      if (currentSession.messages) {
        currentSession.messages.push({ role: 'ai', content: txt, engine: currentSession.engine || 'claude', timestamp: new Date().toISOString() });
        saveCurrentSession();
      }
    } else {
      ...
      (async () => {
        try {
          ...
          if (bg) {
            ...
            bg.messages.push({ role: 'ai', content: txt, engine: bg.engine || 'claude', timestamp: new Date().toISOString() });
            await window.pi.sessionSave(bg);
          }
    ...
    if (knownOrigin === 'task') {
      ...
      _resolveSessionOrigin(sid).then((origin) => {
        if (origin === 'task') finishV2Activity(sid);
          ...
          const st = _uiStates.get(sid);
          if (st?._pendingFinishReason === 'codex-chat-tail') finishV2Activity(sid);
        }
      });
    } else {
      const st = _uiStates.get(sid);
      if (st?._pendingFinishReason === 'codex-chat-tail') {
        finishV2Activity(sid);
      }
    }
    _sessionListStore.setRunning(sid, false);
  ...
  if (payload.type === 'turn-end') {
    const st = _uiStates.get(sid);
    if (!st || !st.activityEl) return;
    st._currentStepGroup = null;
    const history = st.activityEl.querySelector('.exec-history');
    if (history) {
      const last = history.lastElementChild;
      if (!last || !last.classList || !last.classList.contains('exec-turn-divider')) {
        const divider = document.createElement('div');
        divider.className = 'exec-turn-divider';
        divider.textContent = '一轮完成';
  ...
  if (payload.type === 'tool') {
    ...
    if (history) {
      ...
      _updateActivityGroupSummary(st._currentStepGroup, icon, desc);
    }
...
async function sendMessage(input, messagesContainer, options = {}) {
  ...
  const addAI = (text, patch = null) => {
      ...
      (async () => {
        try {
          ...
          if (bg) {
            ...
            bg.messages.push({ role: 'ai', content: text, engine: replyEngine, timestamp: new Date().toISOString() });
            await window.pi.sessionSave(bg);
          }
    ...
    addMessage(messagesContainer, 'ai', text);
    if (currentSession) {
      if (patch) Object.assign(currentSession, patch);
      currentSession.messages.push({ role: 'ai', content: text, engine: replyEngine, timestamp: new Date().toISOString() });
      saveCurrentSession();
    }
    notifyIfHidden(text);
  };
  ...
  if (effectiveEngine === 'claude') {
    ...
    try {
      ...
      if (result && result.aborted) {
        ...
        if (currentSession?.origin !== 'task') {
          ...
          if (st.queuedSends.length === 0) {
            if ((currentSession?.engine || 'claude') === 'codex' && st.stepCount > 0) {
              _schedulePendingActivityFinish(sendSid, 700, 'codex-chat-tail');
            } else {
              finishV2Activity(sendSid);
            }
          }
    ...
    const now = new Date();
    streamLabel.innerHTML = `Pi <span class="msg-time" title="${now.toLocaleString()}">${formatTime(now)}</span>`;
    ...
    const deltaHandler = (delta) => {
      ...
      if (currentEngine === 'auto' && deltaNum <= 8 && /^[\s]*<{1,2}(CLAUDE|EXEC)>{1,2}/i.test(streamText.trim())) {
        ...
        setTimeout(() => {
          ...
          if (displayText) {
            addMessage(messagesContainer, 'ai', displayText);
            if (currentSession) {
              currentSession.messages.push({ role: 'ai', content: displayText, engine: 'gpt', timestamp: new Date().toISOString() });
              saveCurrentSession();
            }
            playTTS(displayText);
          }
          ...
          try {
            ...
            if (result && result.aborted) {
              ...
              if (result.error) {
                ...
                if (finalContent) {
                  addMessage(messagesContainer, 'ai', finalContent);
                  if (currentSession) {
                    currentSession.messages.push({ role: 'ai', content: finalContent, engine: 'exec', timestamp: new Date().toISOString() });
                    saveCurrentSession();
                  }
                }
              }
    ...
    try {
      ...
      if (result.error) {
        ...
        if (currentSession?.id === sendSid) {
          currentSession.messages.push({ role: 'ai', content: finalText, engine: currentEngine, timestamp: new Date().toISOString() });
          saveCurrentSession();
          ...
          (async () => {
            try {
              const bg = await window.pi.sessionLoad(sendSid);
              if (bg) {
                bg.messages.push({ role: 'ai', content: finalText, engine: currentEngine, timestamp: new Date().toISOString() });
                await window.pi.sessionSave(bg);
              }
...
window.pi.onCallPi(async ({ text, title, engine }) => {
  ...
  // NYC 会话只存了 user 消息，Claude 回复却物理写进了 pi-main.messages）
...
window.pi.onPiProactive(async (msg) => {
  ...
  if (currentSession && currentSession.id === MAIN_SESSION_ID) {
    ...
    addMessage(chatMessages, 'ai', text);
...
(async function checkPiOSSetup() {
  ...
  document.getElementById('setup-install').addEventListener('click', async () => {
    ...
    const runtimes = {
      'claude-cli': document.getElementById('setup-claude').checked,
      'codex-cli': document.getElementById('setup-codex').checked,
    };

    const plugins = ['vault', 'shell', 'web-search'];
    if (document.getElementById('setup-browser').checked) plugins.push('browser');
    if (document.getElementById('setup-wechat').checked) plugins.push('wechat');
    if (document.getElementById('setup-health').checked) plugins.push('health');
    if (document.getElementById('setup-photos').checked) plugins.push('photos');
