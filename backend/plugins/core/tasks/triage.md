...
**⚠ 2026-04-29 派发前 dispatch drift 自检（必跑，不可跳）**

排除清单只是"不重复派"。但有些卡进入候选时**根本不该派**（派下去就是 work 空转烧 token），triage 必须自己识别并自修。**对每张候选卡**逐张过下面 4 类检测；命中任一 → 不派 + 写自修动作到 frontmatter + 在收尾 report 写 bullet `- drift-skip {卡名}: {类型}`。命中**不进入**派满逻辑。

1. **completion-drift（实际做完但没收口）**：卡正文 `## 验收标准` 段含 ≥3 条 `- [ ]` / `- [x]` checkbox 且**全部已 `[x]` 勾**
   → 加 `needs_review: completion-drift`（不直接改 done，sense-maker 下轮主观复核）
   不命中即跳过，**不**误标只 1-2 条 checkbox 的小卡

2. **subtree-blocked（剩余子项全 blocked）**：未勾 `- [ ]` 行 ≥1 条且**全部**含 `blocked` / `等待` / `等.*完成` / `need-` 关键词
   → 改 `status: blocked` + 加 `blocked_on: subtree-blocked`（不丢工作，子项解锁后回 active）

3. **cron-stale（cron 卡刚跑过）**：frontmatter 有 `cron:` 字段 + `last_run` 距今 < 2h
   → 跳过本张（cron 自然机制会管下一轮，重派只会让 work 空转）

4. **umbrella-cooldown（伞型今日已消费且无新 owner 输入）**：`type: project` + `parent` ∈ (null/none/空) + 卡内**无任何 checkbox** + `owner_response_consumed_at` 存在 + （`owner_response_at` 不存在 或 `owner_response_at <= owner_response_consumed_at`）+ 当前无 `deferred_until`
   → 加 `deferred_until: <明早 8:00 ISO>`，等明天或新 owner 输入再恢复
   **理由**：伞型卡今天已消费完最新 owner 输入、没新内容、又无 checkbox 子项，反复派只会让 work 写"治理更新摘要"空转。

**自检风险护栏**：

- 4 类自检的判定**只读 frontmatter + 卡正文**，不查 git log / 不调外部命令
- 命中后只动 4 个字段（`needs_review` / `status` / `blocked_on` / `deferred_until`），不删卡 / 不改 body / 不动 ready_for_work
- **拿不准就不命中**——宁可放过让 work 空转一次，也不误标导致正常工作被打断
- 命中报告必须写 bullet（让 owner 在 triage report 里看到自修动作）

**派发量下限**（强制，不保守）：work cron `*/5` 在下一个 15 分钟内 fire 3 次，每次只能消费 1 张。triage 必须派满，让 work 不空转。

- 排除清单后剩余候选 ≥ 3 张 → **必须派满 3 张**
- 候选 < 3 张 → **全部派**
- 候选 = 0 → 派 0 张

**不要做跨卡冲突分析**：worker 有 `claim-before-act` 锁（work.md Step 1）+ 二次确认（work.md Step 1），并发抢锁由锁兜底，不是 triage 的事。triage 误判"它俩可能冲突所以保守只派 1 张" = 越权 + 浪费 work slot（2026-04-29 mainjs-split 教训：3 个 tick 之间 work cron 跑了 ~30 次，全空转）。

排序：`priority` 高 → `energy` 高 → mtime 老。同优先级随机选满 3 张。
...
- 派发：{N} 张 ready_for_work（backlog={M} inflight={K}）/ 无
  # 例：派发：3 张 ready_for_work（backlog=8 inflight=0）— 满载下一个 15min
  # 例：派发：1 张 ready_for_work（backlog=1 inflight=2）— 候选只有 1
  # 单数 1 是异常，除非真的只有 1 张候选，否则违反"派满 3 张"规则
