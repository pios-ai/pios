...
class QwenTTS {
   ...
   */
  async speak(text, timeout = 15000, preset) {
    ...
    try {
      ...
      const wavBuffers = await Promise.all(
        textChunks.map(chunk => this._callTTSAPI(chunk, voice, instruct, timeout))
      );

      const merged = wavBuffers.length === 1
        ? wavBuffers[0]
        : mergeWavBuffers(wavBuffers);
