...
class QwenTTS {
  ...
  async speak(text, timeout = 15000, preset, onChunk) {
    ...
    try {
      ...
      // 串行 = N*per-chunk；首字延迟仍是 1*per-chunk（流式模式下第一段合成完即开播，
      // 后续段在播第一段时合成，audio 时长通常 ≥ 合成时长，用户无感差距）。
      if (typeof onChunk === 'function') {
        // 流式：合成 → filter → 立即回调，按 idx 顺序
        for (let i = 0; i < textChunks.length; i++) {
          const raw = await this._callTTSAPI(textChunks[i], voice, instruct, timeout);
          const filtered = await applyVoiceFilter(raw, filterPreset, magneticLevel).catch(() => raw);
          try { onChunk(filtered, i, textChunks.length); } catch (e) {
