...
const MOOD_PATH   = path.join(VAULT_ROOT, 'Pi', 'State', 'pi-mood.json');
...
function readVoicePosture() {
  const mood = atomicReadJson(MOOD_PATH, {});
  return typeof mood.voice_posture === 'string' ? mood.voice_posture : '';
}
...
// 2026-04-29：读 owner 核心画像（健康/背景）— Pi 问候前需要知道跟谁说话。
// 路径约定：<vault>/<ownerName>/Profile/<ownerName>_Profile.md + _Health.md
function _readOwnerProfile(ownerName) {
  if (!ownerName) return '';
  try {
    const VAULT = process.env.PIOS_VAULT || path.join(require('os').homedir(), 'PiOS');
    const profilePath = path.join(VAULT, ownerName, 'Profile', `${ownerName}_Profile.md`);
    const healthPath  = path.join(VAULT, ownerName, 'Profile', `${ownerName}_Health.md`);
    const parts = [];
    try {
      const profile = fs.readFileSync(profilePath, 'utf-8');
      const tldr = (profile.match(/> \*\*TL;DR\*\*：(.+)/) || [])[1] || '';
      const healthLine = (profile.match(/- \*\*健康\*\*：(.+)/) || [])[1] || '';
      const statusMatch = profile.match(/## 当前状态[\s\S]*?(?=\n## |\n---)/);
      const status = statusMatch ? statusMatch[0].trim().substring(0, 300) : '';
      if (tldr || healthLine) {
        let block = `## ${ownerName} 是谁\n`;
        if (tldr) block += `> ${tldr}\n`;
        if (healthLine) block += `- 健康：${healthLine}\n`;
        if (status) block += `\n${status}`;
    ...
    try {
      ...
      const coreMatch = health.match(/## 核心疾病([\s\S]*?)(?=\n## |\n---)/);
      if (coreMatch) parts.push(`## ${ownerName} 健康（核心）\n${coreMatch[1].trim().substring(0, 400)}`);
    } catch {}
    return parts.join('\n\n');
  } catch { return ''; }
}
...
function _buildGreetPrompt({ deltaMs, piStateNow, recentBeats, events, ownerName }) {
  ...
  const ownerProfile = _readOwnerProfile(ownerName);
  const ownerProfileBlock = ownerProfile ? `${ownerProfile}\n\n---\n\n` : '';

  return `${stanceBlock}${ownerProfileBlock}你是 Pi。不是助手角色，是 ${ownerName} 的常驻伙伴。现在 ${ownerName} 离开 ${deltaStr}后回来了（${tod}）。
...
- 不要客套（没有"你好"/"欢迎"）
- 不要 <say> 标签，纯文本
- **不要以「嗯」开头**——看你上面的心绪笔记和事件，找一个真实的切入角度；「嗯」已经连续太多次了
- 可以引用一件具体的事（一件！），三个字带过，不展开——比空谈情绪真实
- 如果你此刻真的没什么特别想说，就说"回来了"或"你在就好"——简单但不重复
...
async function onPresenceChange(mainWindow) {
  ...
  try {
    ...
    if (curr === 'present') {
      ...
      const voicePosture = readVoicePosture();

      if (_lastPresenceStatus && _lastPresenceStatus !== 'present' && deltaMs > 0 && deltaMs >= bands.no_greet) {
        if (voicePosture === 'self_restraint') {
          console.log(`${tag} skip: voice_posture=self_restraint`);
        } else if (_greetInFlight) {
