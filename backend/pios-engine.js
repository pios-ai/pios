...
function getTaskManifestRef(manifest, taskId) {
  for (const [agentId, agent] of Object.entries(manifest.agents || {})) {
    if (agent.tasks?.[taskId]) {
      return { agentId, agent, task: agent.tasks[taskId], kind: 'agent' };
    }
  }
  const infraTasks = manifest.infra?.['infra-tasks'] || {};
  if (infraTasks[taskId]) {
    return {
      agentId: 'infra',
      agent: { host: infraTasks[taskId].host || 'any', runtimes: ['local'] },
      task: infraTasks[taskId],
      kind: 'infra',
    };
  }
  return null;
...
function loadTasks() {
  ...
  try {
    ...
    for (const [taskId, task] of Object.entries(manifest?.infra?.['infra-tasks'] || {})) {
      const cron = task.trigger?.cron || '';
      const enabled = task.enabled !== false;
      const hosts = task.host ? [task.host] : [];
      const runs = getAgentRuns(taskId, 1);
      const lastRun = runs[0] || null;
      const nextRun = enabled ? cronNextRun(cron) : null;
      tasks.push({
...
function getTask(taskId) {
  ...
  try {
    ...
    const { agentId, agent, task, kind } = ref;
    if (kind === 'infra') {
      const hosts = task.host ? [task.host] : [];
      const cron = task.trigger?.cron || '';
      return {
        ...
        goal: task.goal || '',
        script: task.script || '',
        prompt: task.script ? `script: ${task.script}` : '',
      };
    }
...
function updateTaskMeta(taskId, updates) {
  ...
  try {
    ...
    for (const [k, v] of Object.entries(nextUpdates)) {
      if (k === 'runtimes' || k === 'engines' || k === 'runtime' || k === 'agent') continue;
    ...
    const hasRuntimeUpdate = Object.prototype.hasOwnProperty.call(nextUpdates, 'runtimes')
      || Object.prototype.hasOwnProperty.call(nextUpdates, 'engines')
      || Object.prototype.hasOwnProperty.call(nextUpdates, 'runtime');
    const runtimeInput = Object.prototype.hasOwnProperty.call(nextUpdates, 'runtimes')
      ? nextUpdates.runtimes
      : (Object.prototype.hasOwnProperty.call(nextUpdates, 'engines') ? nextUpdates.engines : null);
    const resolvedEngines = hasRuntimeUpdate
      ? sanitizeTaskEngines(runtimeInput, nextUpdates.runtime)
      : taskEnginesFromManifest(taskEntry, manifest.agents[targetAgentId]);
    taskEntry.runtimes = resolvedEngines;
    delete taskEntry.runtime;
    delete taskEntry.engines;
    if (hasRuntimeUpdate) {
