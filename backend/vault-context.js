...
/** 读取 owner 核心画像（健康 + 背景摘要）— 让 Pi 知道跟谁在说话。
 *  路径约定：<vault>/<ownerName>/Profile/<ownerName>_Profile.md + _Health.md */
function getOwnerProfileContext() {
  const ownerName = getOwnerName();
  if (!ownerName) return null;
  const parts = [];
  const profile = readFile(`${ownerName}/Profile/${ownerName}_Profile.md`);
  if (profile) {
    const tldr = (profile.match(/> \*\*TL;DR\*\*：(.+)/) || [])[1] || '';
    const healthLine = (profile.match(/- \*\*健康\*\*：(.+)/) || [])[1] || '';
    const statusMatch = profile.match(/## 当前状态[\s\S]*?(?=\n## |\n---)/);
    const status = statusMatch ? statusMatch[0].trim().substring(0, 400) : '';
  ...
  const health = readFile(`${ownerName}/Profile/${ownerName}_Health.md`);
  if (health) {
    const coreMatch = health.match(/## 核心疾病([\s\S]*?)(?=\n## |\n---)/);
    if (coreMatch) parts.push(`## ${ownerName} 健康（核心）\n${coreMatch[1].trim().substring(0, 500)}`);
  }
...
- 不要自己搜索网络，除非 ${owner} 明确要求`];
  ...
  // 2026-04-29：owner 画像 — Pi 需要知道跟谁说话（健康/背景/当前状态）
  const ownerProfile = getOwnerProfileContext();
  if (ownerProfile) parts.push(ownerProfile);
...
module.exports = { VAULT_PATH, readFile, buildSystemContext, getOwnerName, getBootContext, getActiveCards, getHeartbeatContext, getOwnerProfileContext };
