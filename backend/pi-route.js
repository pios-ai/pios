...
// ── mirror 到派总 weixin direct session ──
// 2026-04-29：从 notify-wechat.sh 提到 pi-route 这一层。任何 owner-facing 推送
// 都应该 mirror（让派总知道自己说过 X），不只是 wechat 通道成功时。
// 软失败 + 写 mirror-failures.log，不阻塞 send 路径。
const MIRROR_FAILURES_LOG = path.join(VAULT_ROOT, 'Pi', 'Log', 'mirror-failures.log');
const PIOS_CONFIG_PATH = path.join(require('os').homedir(), '.pios', 'config.json');
let _piosConfigCache = null;
function _loadPiosConfig() {
  if (_piosConfigCache) return _piosConfigCache;
  try { _piosConfigCache = JSON.parse(fs.readFileSync(PIOS_CONFIG_PATH, 'utf8')); } catch { _piosConfigCache = {}; }
  return _piosConfigCache;
}
function mirrorToOwnerSession(text) {
  if (process.env.PIOS_TEST_MODE === '1') return;
  try {
    const cfg = _loadPiosConfig();
    const ownerTarget = cfg.openclaw_target || '';
    const remoteHost = cfg.openclaw_remote || '';  // e.g. user@hostname with openclaw sessions
    if (!ownerTarget || !remoteHost) return; // not configured
    const sessionKey = `agent:main:openclaw-weixin:direct:${ownerTarget.toLowerCase()}`;
    const hash = crypto.createHash('sha256').update(text).digest('hex').slice(0, 16);
    const idempKey = `pios-notify-${hash}`;
    const payload = JSON.stringify({ sessionKey, text, idempotencyKey: idempKey });
    // remote worker has openclaw + sessions.json; must ssh (local host has no openclaw sessions/)
    execSync(`ssh -o ConnectTimeout=5 ${remoteHost} "node ${cfg.vault_root || '/data/AI_Vault'}/Pi/Tools/oc-mirror-append.mjs"`, {
      input: payload,
      timeout: 10000,
    ...
    try {
      fs.appendFileSync(MIRROR_FAILURES_LOG,
        `${new Date().toISOString()} [pi-route mirror fail] ${(e.message || '').slice(0, 200)}\n`);
    } catch {}
  }
}
...
async function send({ text, level = 'info', source = 'unknown', audience = 'owner', mainWindow = null, expires_at = null, ts = null } = {}) {
  ...
  if (presence.status === 'present') {
    // ⚠️ HARD RULE 2026-04-29（"在电脑前就不用发微信"，参 Cards/active/verify-pi-route-present-no-wechat-2026-04-29.md）：
    // present 状态 = 任何 level 都不发微信。本地 notify + bubble 是足够通道。
    // 历史：2026-04-26 commit 51d255d 加的 "report→WeChat 硬 gate" 设计——把
    // present + report 也补一份微信作为"手机 backup"，owner 反馈这是噪音。
    // mainWindow=null 时无 bubble = 单纯本地 notify（macOS 通知中心）；不再
    // 靠微信兜底（那是另一处独立 bug，要修 mainWindow 注册路径，不靠微信掩盖）。
    const r1 = sendLocalNotify(level, text, effectiveExpIso);
    const r2 = mainWindow ? sendLocalBubble(mainWindow, text) : null;
    // ⚠️ 2026-04-29（"刚刚这个通知，为什么 Pi-Main 自己又不知道！？？？"）：
    // 之前 mirror 绑在 notify-wechat.sh 内（wechat send 成功后才 mirror）。撤掉
    // present+report 的 sendWeChat 后顺带撤了 mirror → 派总永远不知道。
    // 修：mirror 是"PiOS 对 owner 说话"的独立动作，不该绑在某一通道上。任何
    // routed='local-present' 也 mirror 一次（idempotency 由 oc-mirror-append.mjs
    // grep 保证不重复）。
    if (audience === 'owner') mirrorToOwnerSession(text);
