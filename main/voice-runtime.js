...
/**
 * 检测系统是否有 launchctl-managed qwen-voice plist。
 * 有的话 PiOS 不自起 — 让 launchctl 一家管，避免两个 spawner 抢端口（2026-04-30
 * SIGABRT 教训：launchctl ThrottleInterval=10s 重启缝隙里 PiOS HTTP probe
 * 失败就自 spawn，launchctl 同时复活，两个 python 抢 :7860 → fatal_error abort）。
 */
function _hasLaunchctlPlist() {
  try {
    const home = process.env.HOME || '';
    if (!home) return false;
    const dir = path.join(home, 'Library', 'LaunchAgents');
    if (!fs.existsSync(dir)) return false;
    // Glob match any com.<anything>.qwen-voice.plist — works for any owner who
    // followed the activate.md plist convention; not tied to one specific username.
    return fs.readdirSync(dir).some(f => /qwen-voice\.plist$/.test(f));
  } catch { return false; }
}

/** 先查 7860 是否已经活（可能是外部装的）；没活才决定要不要自 spawn */
function ensureQwenStarted() {
  return new Promise((resolve) => {
    const _onDeadPort = () => {
      // 系统级 plist 在管 → 让位，等 launchctl ThrottleInterval(10s) 自己 revive
      if (_hasLaunchctlPlist()) {
        return resolve({ ok: false, reason: 'deferring to launchctl-managed qwen-voice (will auto-restart)' });
      }
      // 没 plist → 走原本的自 spawn 兜底（新用户、SSH 装机、跳过激活引导的场景）
      resolve(startQwenVoiceService());
    };
    ...
    req.on('error', _onDeadPort);
    req.on('timeout', () => { try { req.destroy(); } catch {} _onDeadPort(); });
    req.end();
  });
}
