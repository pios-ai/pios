  ...
  ipcMain.handle('session:ensure', async (_, { sessionId, engine, claudeSessionId, codexThreadId }) => {
    try {
      ...
      if (existing && existing.engine === engine) {
        // Safe idempotent re-attach: if the adapter lost in-memory identity or
        // the renderer has since persisted a thread/session id, give it another
        // chance to bind without destroying the running engine.
