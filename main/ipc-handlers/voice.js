  ...
  ipcMain.handle('voice:tts', async (evt, text, preset) => {
    const task = _voiceTTSChain.then(async () => {
      try {
        ...
        // 物理消除"双 enqueue"风险：永远 return null，所有 chunk 都通过 voice:tts:chunk 事件
        // 流到 renderer audioQueue。若任何 caller 还在 enqueue 这个 return，那只是 enqueue null
        // 被 byteLength 检查挡掉。2026-04-30 教训：onNotifySpeak 旧 pattern enqueue return,
        // 跟 chunk 事件 last chunk 撞，短通知念 2 次。
        return null;
      } catch (err) {
